Ext.onReady(() => {
	...
	let registerUserStore = Ext.create('Ext.data.Store', {
		...
		proxy: {
			type: 'ajax',
			url: 'http://localhost:8080/HighwayToIRProjectSpringStrutsJDBC/GetAllRegisteredUser',
			reader: {
				type: 'json',
									...
									if (confirmpassword_input == newUser.password) {
										...
										if (availableDataFromStore !== null) {
											...
											Ext.Ajax.request({
												...
												url: `http://localhost:8080/HighwayToIRProjectSpringStrutsJDBC/RegisterNewUser`,
												jsonData: newUser,
												success: (response) => {
